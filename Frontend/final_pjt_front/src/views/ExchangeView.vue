<template>
  <div class="exchange-rates-container">
    ...
    <ul class="cny">
      <li>
        '최종고시환율'은 오늘 기준 가장 최근 고시된 환율정보를 나타내며, 은행의
        환율고시가 종료되지 않은 상태일 수 있습니다.
      </li>
      <li>
        중국 위안화(CNY)는 홍콩 외환시장에서 거래되는 위안화 환율정보(CNH)를
        고시합니다.
      </li>
    ...
    <div v-if="selectedExchangeRate" class="exchange-rate-details">
      <div class="helpme">
      <h1>{{ selectedExchangeRate.cur_unit }}</h1>
      <!-- 외화 살 때, 외화 팔 때 버튼 -->
      ...
      <div v-show="selectedDiv === 1" class="calculation-container" id="1">
        <div class="input-container">
          <label for="amountKRW" class="input-label"
            >지불해야 할 금액(원화)</label
          >
          <div class="input-box-with-unit">
            <input
              type="number"
              v-model="wanttobuyKRW"
              id="amountKRW"
              class="input-box"
            />
        ...
        <div class="input-container">
          <label for="calculationMethodKRW" class="input-label"
            >계산 방식</label
          >
          <select
            v-model="calculationMethodKRW"
            id="calculationMethodKRW"
            class="input-box"
          >
        ...
        <div class="result-container">
          <span class="label">살 수 있는 금액(외화)</span>
          <span class="value"
            >{{ formattedResultKRW }} {{ selectedCurrencyUnit }}</span
          >
        </div>
      </div>

      <div v-show="selectedDiv === 2" class="calculation-container" id="2">
        <div class="input-container">
          <label for="amount" class="input-label">갖고 있는 금액(외화)</label>
          <div class="input-box-with-unit">
            <input
              type="number"
              v-model="wanttobuy"
        ...
        <div class="input-container">
          <label for="calculationMethod" class="input-label">계산 방식</label>
          <select
        ...
        <div class="result-container">
          <span class="label">받으실 금액(원화)</span>
...
const wanttobuyKRW = ref(null); // 원화 입력값
const calculationMethod = ref("deal_bas_r");
const calculationMethodKRW = ref("deal_bas_r");
...
const fetchExchangeRate = async (curUnit, curUnitName) => {
  ...
  try {
    const response = await axios.get(
      `http://127.0.0.1:8000/exchange/${curUnit}/`
    );
...
const calculateResult = () => {
  ...
  const rate = Number(
    selectedExchangeRate.value[calculationMethod.value].replace(/,/g, "")
  ); // 콤마 제거 후 숫자로 변환
...
const calculateResultKRW = () => {
  if (!selectedExchangeRate.value || !wanttobuyKRW.value) return 0; // 값이 null이면 계산하지 않음
  const rate = Number(
    selectedExchangeRate.value[calculationMethodKRW.value].replace(/,/g, "")
  ); // 콤마 제거 후 숫자로 변환
  const amount = parseFloat(wanttobuyKRW.value); // 입력된 값이 숫자인지 확인
  if (isNaN(amount)) {
    // 숫자가 아닌 경우
    error.value = "올바른 숫자를 입력하세요."; // 오류 메시지 표시
    return 0;
  }
  return amount / rate; // 원화에서 외화로 변환
};

const formattedResult = computed(() => {
  const result = calculateResult();
  return new Intl.NumberFormat().format(result) + "원";
});

const formattedResultKRW = computed(() => {
  const result = calculateResultKRW();
...
.case-button {
    margin-left: 10px;
    background-color: #fff; /* 기본 색상은 하얀색 */
    color: #000; /* 텍스트 색상은 검은색 */
    border: 1px solid #000; /* 테두리는 검은색 */
    transition: background-color 0.3s, color 0.3s; /* 전환 효과 */
    border-radius: 10px;
  }

  .case-button:hover {
    background-color: #0056b3; /* 마우스를 올렸을 때의 배경색 */

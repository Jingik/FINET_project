...
async function fetchProducts() {
  try {
    ...
    wishlist.value = wishlistRes.data.deposit_subscriptions.map(item => item.deposit_product);
  } catch (error) {
...
function toggleWishlist(productId) {
  if (wishlist.value.includes(productId)) {
    axios.post('http://127.0.0.1:8000/financial/favorites/remove_deposit/', { deposit_product_id: productId })
      .then(() => {
    ...
    axios.post('http://127.0.0.1:8000/financial/favorites/add_deposit/', { deposit_product_id: productId })
      .then(() => {

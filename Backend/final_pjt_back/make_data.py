...
first_name_samples = '김이박최정강조윤장임한'
middle_name_samples = '민서예지도하주윤채현지'
last_name_samples = '준윤우원호후서연아은진아'

phone_number_samples = ['010', '011', '016', '017', '018', '019']
...
def random_phone_number():
    return random.choice(phone_number_samples) + ''.join(random.choice('0123456789') for _ in range(8))
...
save_dir = './dummydata/user_data.json'
with open(save_dir, 'w', encoding="utf-8") as f:
    ...
    for i in range(N):
        ...
        file['fields'] = {
            ...
            'password': '1234',
            'name': username_list[i],  # 유저 이름 랜덤 생성
            'phone_number': random_phone_number(), # 폰 번호
            'email': f'user_{i+1}@example.com', # 이메일
            'user_age_group': random.choice(['10대', '20대', '30대', '40대', '50대']), #연령대
            'service_purpose': random.choice(['예금 가입', '적금 가입', '대출 가입']), #가입 목적
            'asset': random.randrange(0, 100000000, 100000),  # 자산
